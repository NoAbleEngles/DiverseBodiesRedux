...
namespace utils
{
	RandomGenerator::RandomGenerator()
	{
		...
		for (size_t i = 0; i < num_generators; ++i) {
			gen[i].seed(static_cast<unsigned int>(std::random_device{}()) + static_cast<unsigned int>(i));  // Используем разные семена
		}
	}
	...
	void RandomGenerator::update_seed(unsigned int new_seed)
	{
		...
		std::lock_guard<std::mutex> lock(mutex);  // Защита от одновременного доступа
		for (size_t i = 0; i < num_generators; ++i) {
			gen[i].seed(static_cast<unsigned int>(new_seed) + static_cast<unsigned int>(i));  // Обновляем семена для каждого генератора
		}
	}
}
