# Система адаптивного меню для Fallout 4 / Scaleform

Современная система меню для интеграции с Fallout 4 через Scaleform (Flash/ActionScript 3), адаптированная для работы с DirectApply меню.

## 🚀 Ключевые особенности

### ✨ Новый концепт дизайна
- **Единственный ScrollPane** - только скроллируемая панель без фона и заголовка
- **30% ширины экрана** - компактное размещение справа
- **Адаптивное позиционирование** - правый край с 10px отступами
- **Полная высота экрана** - максимальное использование пространства

### 🎯 Интеграция с C++
- **Простой C++ API** - push, pushArray, insert, find, clear
- **Polling колбэки** - надежная передача событий от Flash к C++
- **Типизированные данные** - безопасная передача через переменные уровня _root
- **Совместимость со старыми форматами** - обратная совместимость

### 🔧 Поддерживаемые компоненты
- **Button** - интерактивные кнопки
- **Checkbox** - переключатели вкл/выкл  
- **Switcher** - выпадающие списки
- **Label** - текстовые заголовки и разделители

### 🔄 Polling механизм колбэков
- **Надежная доставка** - колбэки всегда доходят до C++, даже если ExternalInterface недоступен
- **_root переменные** - Flash устанавливает `_root.pendingCallback` и `_root.callbackArgs`
- **Автоматическая проверка** - C++ проверяет переменные каждый кадр в ProcessMessage(kUpdate)
- **Самоочистка** - переменные автоматически очищаются после обработки
...
│           └── LabelComponent.as
└── CustomEvent.as (Система событий)
```
...
- ✅ **@brief описания для всех публичных методов**
...
- **Components**: полностью документированные UI элементы
